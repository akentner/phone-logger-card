...
/**
  ...
  `}function $t(t,e){return"answered"===t?"inbound"===e?{icon:"mdi:phone-incoming",color:"#3092dc"}:{icon:"mdi:phone-outgoing",color:"#8bbf68"}:"missed"===t?{icon:"mdi:phone-remove",color:"#e45f3b"}:"not_reached"===t?{icon:"mdi:phone-remove",color:"#8bbf68"}:"voicemail"===t?{icon:"mdi:phone-message",color:"#e45f3b"}:{icon:"mdi:phone-hangup",color:"var(--secondary-text-color, grey)"}}function yt(t){if(!t||t<=0)return"–";const e=Math.floor(t/3600),s=Math.floor(t%3600/60),i=t%60;return`${String(e).padStart(2,"0")}:${String(s).padStart(2,"0")}:${String(i).padStart(2,"0")}`}function vt(t,e){return t?new Date(t).toLocaleString(e,{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"}):"–"}function bt(t){return`${t.getFullYear()}-${t.getMonth()}-${t.getDate()}`}function At(t,e,s){const i=bt(e),o=bt(t),n=new Date(e.getFullYear(),e.getMonth(),e.getDate()),r=new Date(t.getFullYear(),t.getMonth(),t.getDate()),a=Math.round((n.getTime()-r.getTime())/864e5);return o===i?_t("today",s):1===a?_t("yesterday",s):2===a?_t("two_days_ago",s):t.toLocaleDateString(s,{weekday:"short",day:"2-digit",month:"2-digit",year:"numeric"})}class wt extends at{constructor(){super(...arguments),this._calls=[],this._nextCursor=null,this._loading=!1,this._loadingMore=!1,this._error=null,this._selectedCall=null,this._failCount=0,this._circuitOpenUntil=0}get _lang(){return this.hass?.language}static{this.CIRCUIT_MAX_FAILURES=3}static{this.CIRCUIT_COOLDOWN_MS=3e5}static getStubConfig(){return{}}setConfig(t){this._config=t}connectedCallback(){super.connectedCallback(),this._startPolling()}disconnectedCallback(){super.disconnectedCallback(),this._stopPolling()}updated(t){t.has("hass")&&this.hass&&0===this._calls.length&&!this._loading&&this._fetchCalls()}_startPolling(){this._stopPolling(),this._pollTimer=setInterval(()=>this._fetchCalls(),6e4)}_stopPolling(){this._pollTimer&&(clearInterval(this._pollTimer),this._pollTimer=void 0)}async _fetchCalls(t){if(!this.hass)return;if(Date.now()<this._circuitOpenUntil)return;const e=Boolean(t);e?this._loadingMore=!0:(this._loading=!0,this._error=null);try{const s=this._config?.limit??20,i=this._config?.msn?Array.isArray(this._config.msn)?this._config.msn:[this._config.msn]:[],o=await this.hass.callWS({type:"phone_logger/calls",limit:s,...t?{cursor:t}:{},...i.length?{msn:i}:{}});this._calls=e?[...this._calls,...o.items??[]]:o.items??[],this._nextCursor=o.next_cursor??null,this._failCount=0}catch(t){this._failCount++;const e=t instanceof Error?t.message:"object"==typeof t&&null!==t&&"message"in t?String(t.message):JSON.stringify(t);this._failCount>=wt.CIRCUIT_MAX_FAILURES?(this._circuitOpenUntil=Date.now()+wt.CIRCUIT_COOLDOWN_MS,this._error=e+_t("retry_suffix",this._lang),this._failCount=0):this._error=e}finally{this._loading=!1,this._loadingMore=!1}}_loadMore(){this._nextCursor&&this._fetchCalls(this._nextCursor)}_groupedCalls(){const t=new Date,e=new Map;for(const t of this._calls){const s=bt(new Date(t.started_at));e.has(s)||e.set(s,[]),e.get(s).push(t)}return Array.from(e.entries()).map(([,e])=>({day:At(new Date(e[0].started_at),t,this._lang),calls:e}))}render(){if(!this._config)return q;const t=this._groupedCalls(),{title:e}=this._config;return B`
    ...
    `}_renderRow(t){const e=$t(t.status,t.direction),s=mt(t.status,t.direction,this._lang),i="inbound"===t.direction?t.caller_display:t.called_display,o=t.caller_device?.name??null,n=new Date(t.started_at).toLocaleTimeString(this._lang,{hour:"2-digit",minute:"2-digit"}),r=o?`${n} – ${o}`:n;return B`
      <tr
    ...
    `}_renderModal(t){const e=this._lang,s=$t(t.status,t.direction),i=mt(t.status,t.direction,e),o="inbound"===t.direction?t.caller_display:t.called_display,n=t.caller_device,r=(t,e)=>B`
      <tr>
      ...
      <div class="modal-backdrop" @click=${this._closeModal}>
        <div class="modal" @click=${t=>t.stopPropagation()}>
          ...
          <table class="modal-table">
            ${r(_t("modal_caller",e),`${t.caller_display} (${t.caller_number})`)}
            ${r(_t("modal_called",e),`${t.called_display} (${t.called_number})`)}
            ${n?r(_t("modal_device",e),n.name):q}
            ${n?r(_t("modal_extension",e),n.extension):q}
            ${r(_t("modal_started",e),vt(t.started_at,e))}
            ${r(_t("modal_connected",e),vt(t.connected_at,e))}
            ${r(_t("modal_finished",e),vt(t.finished_at,e))}
            ${r(_t("modal_duration",e),yt(t.duration_seconds))}
            ${r(_t("modal_msn",e),t.msn||_t("modal_unknown",e))}
            ${r(_t("modal_trunk",e),t.trunk_id||_t("modal_unknown",e))}
            ${r(_t("modal_line",e),String(t.line_id))}
            ${r(_t("modal_internal",e),t.is_internal?_t("modal_yes",e):_t("modal_no",e))}
          </table>
        </div>
      </div>
    `}_closeModal(){this._selectedCall=null}static{this.styles=r`
    :host {
      display: block;
    }
    .card-header {
  ...
  `}}t([ht({attribute:!1})],wt.prototype,"hass",void 0),t([pt()],wt.prototype,"_config",void 0),t([pt()],wt.prototype,"_calls",void 0),t([pt()],wt.prototype,"_nextCursor",void 0),t([pt()],wt.prototype,"_loading",void 0),t([pt()],wt.prototype,"_loadingMore",void 0),t([pt()],wt.prototype,"_error",void 0),t([pt()],wt.prototype,"_selectedCall",void 0),window.customCards=window.customCards||[],window.customCards.push({type:"phone-logger-card",name:"Phone Logger Card",description:"Displays phone call history via Phone Logger custom component",preview:!1,documentationURL:"https://github.com/akentner/phone-logger-card"}),customElements.define("phone-logger-card",wt),console.info("%c PHONE-LOGGER-CARD %c v2.0.0 ","color:white;background:#3092dc;font-weight:bold","color:#3092dc;background:white;font-weight:bold")}();
