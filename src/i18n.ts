...
export function statusLabel(status: string, direction: string, language?: string): string {
  if (status === 'answered') {
    return direction === 'inbound' ? t('status_answered_inbound', language) : t('status_answered_outbound', language);
  }
  if (status === 'missed') return t('status_missed', language);
  if (status === 'not_reached') return t('status_not_reached', language);
  if (status === 'voicemail') return t('status_voicemail', language);
  return t('status_default', language);
}
